...
static int lx_open (lua_State *L) {
 // luaL_newmetatable(L,MYTYPE); --> Use the module table instead
 lua_pushvalue(L, 2);
 lua_pushstring(L, MYTYPE);
 lua_setfield(L, -2, "__name");  /* metatable.__name = MYTYPE */
 lua_pushvalue(L, -1);
 lua_setfield(L, LUA_REGISTRYINDEX, MYTYPE);  /* registry.name = metatable */
